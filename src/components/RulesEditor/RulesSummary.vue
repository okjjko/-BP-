<template>
  <!--
    规则只读摘要组件（多人模式 / 对局中通用）。常驻 BP 头部单行胶囊：
      - 小局后选边权（loserPickMode 中文文案）
      - 复选同一植物上限（maxPlantUsage）
      - BP 流程概要（阶段数 + 总步数，点击展开完整流程弹窗）
    所有角色（host / 选手 / 观众）均可见，仅用于查看当前生效规则；窄屏自动 flex-wrap。
  -->
  <div
  ...
  >
    ...
    <!-- 复选同一植物上限（每名选手独立计数） -->
    <span class="chip">
      <span class="lbl lbl--wide" title="复选同一植物上限">复选同一植物上限</span>
    ...
    <button
      ...
      @click="showFlow = true"
    >
      <span class="val">显示 BP 顺序</span>
      <ChevronRight :size="12" class="link-icon" aria-hidden="true" />
    </button>

    <!-- 流程详情弹窗（移动端近全屏，内容可滚动；Esc / 点遮罩可关闭） -->
    <BaseDialog
      v-model="showFlow"
      title="BP 流程"
