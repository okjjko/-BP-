...
如果启动时提示 "3000端口已被占用" 或类似错误，说明之前的开发服务器没有正确关闭。解决方法：
...
# 输出示例：TCP    0.0.0.0:3000    0.0.0.0:0    LISTENING    21452
# 最后的数字就是进程ID（PID）
...
# 输出示例：node   12345 user   21u  IPv6   ...  TCP *:3000 (LISTEN)
# 第二列的数字就是进程ID（PID）
...
> ℹ️ 上述路径为默认示例，实际以你服务器的部署位置为准（本文档统一以 `/var/www/bp-tool` 为例）。
...
- 完整的待办与规划清单见 [docs/TODO.md](docs/TODO.md)
